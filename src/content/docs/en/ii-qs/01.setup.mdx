...
:::caution[Hyprland version]
This targets Hyprland 0.56, which configures itself in Lua. The Hyprland config shipped here is Lua-only, so an older Hyprland will not read it.
:::
...
- **WIP:**
  - The work-in-progress `dist-nix` targets NixOS through Nix and home-manager.
...
```bash
bash <(curl -s https://kitty-hivens.github.io/YukiUI-wiki/get)
```

It clones the repo to `~/.cache/YukiUI` by default and run `./setup install` for you.

### Advanced: Customize installation
To clone to somewhere else, just append the path, e.g.
```bash
bash <(curl -s https://kitty-hivens.github.io/YukiUI-wiki/get) ~/Downloads/YukiUI
```
...
```bash
bash <(curl -s https://kitty-hivens.github.io/YukiUI-wiki/get) -- --core
```
To combine the two behaviors above, run
```bash
bash <(curl -s https://kitty-hivens.github.io/YukiUI-wiki/get) ~/Downloads/YukiUI -- --core
```
...
git clone https://github.com/Kitty-Hivens/YukiUI # Clone the repo
cd YukiUI                                        # Go to the cloned repo
...
```bash
t=~/.cache/YukiUI   # Let's not trash your home folder
git clone https://github.com/Kitty-Hivens/YukiUI.git "$t" --filter=blob:none --recurse-submodules
cd "$t"
```
...
- Figure out the equivalents of the packages listed in [deps-info.md](https://github.com/Kitty-Hivens/YukiUI/blob/main/sdata/deps-info.md) and install them.
...
- Launch the uwsm-managed session. `hyprland/execs.lua` runs `uwsm finalize` and starts the shell as a `uwsm app` unit, so a plain Hyprland session comes up without the shell.
- Side effect of a systemd-managed session: its processes belong to no logind session, which makes polkit treat them as inactive. That is why `49-vt-switch.rules` is installed -- without it `Ctrl`+`Alt`+`F<n>` stops switching consoles.
...
```bash
cd ~/.cache/YukiUI && git stash && git pull && ./setup install
```
:::
...
1. `cd` into the repo directory. By default it should be on `~/.cache/YukiUI`, unless you cloned the repo somewhere else.
